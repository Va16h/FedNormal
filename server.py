...
class Server(object):
    ...
    def update_selected_clients(self, sampled_client_indices):
        ...
        for idx in tqdm(sampled_client_indices, leave=False):
            self.clients[idx].client_update(self.server_controls, self.model)
    ...
    def mp_update_selected_clients(self, selected_index):
        ...
        self.clients[selected_index].client_update(self.server_controls, self.model)
    ...
    # p denotes nk / n
    def average_model(self, sampled_client_indices, p):
        ...
        # Outer Normalizing Coeffecient
        # Can loop over clients here to calculate this        

        sum_of_delta_c = [torch.zeros_like(p.data) for p in self.model.parameters() if p.requires_grad]

        client_avg_params = None

        for i, idx in tqdm(enumerate(sampled_client_indices), leave=False): 
            if (client_avg_params == None):
                client_avg_params = self.clients[idx].client_params
            else:                
                client_avg_params += self.clients[idx].client_params

        old_model_params = torch.nn.utils.parameters_to_vector(self.model.parameters())

        client_avg_params /= len(sampled_client_indices)

        c = client_avg_params - old_model_params

        normalizing_coeff = getattr(registry[self.optimizer], 'get_normalizing_coeff')(self.clients, sampled_client_indices, c)

        for i, idx in tqdm(enumerate(sampled_client_indices), leave=False): 
            local_weights = self.clients[idx].model.state_dict()

            for p_idx in range(len(sum_of_delta_c)):
                sum_of_delta_c[p_idx].data += self.clients[idx].delta_controls[p_idx].data

            # extra coeff to multiply to nk/n like tau_i
            inner_coeff = self.clients[idx].optimizer_object.get_inner_coeff(self.clients[idx].c_i, c)

            for key in self.model.state_dict().keys():
                if i == 0:
                    averaged_weights[key] = normalizing_coeff * p[i] * inner_coeff * local_weights[key]
                else:
                    averaged_weights[key] += normalizing_coeff * p[i] * inner_coeff * local_weights[key]
        ...
        for p_idx in range(len(sum_of_delta_c)):             
            self.server_controls[p_idx].data = self.server_controls[p_idx].data + sum_of_delta_c[p_idx].data / len(sampled_client_indices)
